...
# -*- encoding: utf-8 -*-
'''
@Id :generate_calculation_scripts.py
@date :2025/01/27
@author :YenochQin (秦毅)
@description: 生成GRASP计算的配置文件和运算脚本
'''
...
class CalculationScriptGenerator:
    """GRASP计算脚本生成器"""
    ...
    def generate_config_toml(self, config_params: Dict[str, Any]) -> str:
        ...
        for key, value in config_params.items():
            ...
            elif key == "model_params" and isinstance(value, dict):
                default_config["model_params"].update(value)
        
        # 生成配置文件内容
        config_content = f"""# GRASP计算配置文件
# 生成时间: {config_params.get('generation_time', '')}

atom = "{default_config['atom']}"
conf = "{default_config['conf']}"
spetral_term = {default_config['spetral_term']}
continue_cal = {str(default_config['continue_cal']).lower()}
cal_loop_num = {default_config['cal_loop_num']}
cal_error_num = {default_config['cal_error_num']}
cal_method = "{default_config['cal_method']}"
difference = {default_config['difference']}
cutoff_value = {default_config['cutoff_value']}
initial_ratio = {default_config['initial_ratio']}
expansion_ratio = {default_config['expansion_ratio']}
target_pool_file = "{default_config['target_pool_file']}"
root_path = "{default_config['root_path']}"
selected_csfs_file = "{default_config['selected_csfs_file']}"
selected_csfs_mix_file = "{default_config['selected_csfs_mix_file']}"

# 收敛性检查参数
std_threshold = {default_config['std_threshold']}  # 标准差阈值
relative_std_threshold = {default_config['relative_std_threshold']}  # 相对标准差阈值

[model_params]
n_estimators = {default_config['model_params']['n_estimators']}
random_state = {default_config['model_params']['random_state']}

[model_params.class_weight]
0 = {default_config['model_params']['class_weight']['0']}
1 = {default_config['model_params']['class_weight']['1']}
"""
    ...
    def generate_calculation_script(self, script_params: Dict[str, Any]) -> str:
        ...
        # 默认脚本参数
        default_params = {
            "job_name": "GdIoddImlcias4_odd2",
            "node_count": 1,
            "tasks_per_node": 46,
            "partition": "work3",
            "output_log": "as4_odd2GdIoddImlci.log",
            "processor": 46,
            "atom": "GdI",
            "conf": "cv4odd1as4_odd2",
            "varied": "3",
            "loop1_rwfn_file": "mJ-1-90chosenas3_odd2.w",
            "conda_path": "/home/workstation3/AppFiles/miniconda3/etc/profile.d/conda.sh",
            "conda_env": "grasp-env",
            "grasp_path": "/home/workstation3/AppFiles/GraspDataProcessing/src"
        }
...
#SBATCH -J {default_params['job_name']}
#SBATCH -N {default_params['node_count']}
#SBATCH --ntasks-per-node={default_params['tasks_per_node']}
#SBATCH -p {default_params['partition']}
#SBATCH --output={default_params['output_log']}
. /usr/share/Modules/init/zsh
###########################################
# mpi run CPU core
processor={default_params['processor']}
###########################################
## module load
...
###########################################
## configuration
atom={default_params['atom']}
conf="{default_params['conf']}"
varied="{default_params['varied']}"
loop1_rwfn_file="{default_params['loop1_rwfn_file']}"
...
def main():
    ...
    parser.add_argument('--job-name', default='GdIoddImlcias4_odd2', help='作业名称')
    ...
    config_params = {
        'atom': args.atom,
        'conf': args.conf,
        'spetral_term': [args.spectral_term],
    ...
    # 生成文件
    generator = CalculationScriptGenerator(args.output_dir)
