...
def main(cfg):
    ...
    while step > 0:
        ...
        if mc_step == 0:
            if step == 1:
                ...
                if initial_method == "designation":
                    ...
                    vmc.save_ci(index, path + file_name + str(step) + ".c", N_ci, cis_ts, head)#保存文件操作
        ...
        start_time = time.time()
        os.system("echo y | mpirun -hostfile hostfile -np 12 -mca btl ^openib rangular_mpi")
...
if __name__ == "__main__":
    ...
    cfg = {
        ...
        "initial_ratio": 0.02,#初始化比例
        "expansion_ratio": 2,#拓展比例，下一次实际计算的组态数N=expansion_ratio*重要组态
        "target_pool_file": "pool.c",#目标组态空间文件
        "root_path": "/home/workstation1/8thdd",
        "spetral_term": ['3s(2).3p(6).3d(10)1S0_1S.4s(2).4p(2)3P2_3P',
                    '3s(2).3p(6).3d(10)1S0_1S.4s(2).4p(2)1S0_1S',
                    '3s(2).3p(6).3d(10)1S0_1S.4p(4)1S0_1S',
                    '3s(2).3p(6).3d(10)1S0_1S.4s_2S.4p(2)3P2_4P.4d_5D',
                    '3s(2).3p(6).3d(10)1S0_1S.4s_2S.4p(2)1D2_2D.4d_3P',
                    '3s(2).3p(6).3d(10)1S0_1S.4s_2S.4p(2)3P2_4P.4d_3P',
                    '3s(2).3p(6).3d(10)1S0_1S.4s_2S.4p(2)1D2_2D.4d_1S'],
    }
    cfg['root_path'] = f"/home/workstation1/8thdd/recode_mlsci/mlsic_recode_v2/NdGelike_block1-3/{cfg['atom']}_{cfg['conf']}_{cfg['block']}"
    cfg['target_pool_file'] = f'evencv3pas5_even{cfg['block']}.c'
